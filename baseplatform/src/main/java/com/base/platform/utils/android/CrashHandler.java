...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private boolean handleException(final Throwable ex) {
        ...
        //友盟错误上传
        MobclickAgent.reportError(BaseApplication.getInstance().getApplicationContext(),ex);
        // 使用Toast来显示异常信息
        new Thread() {
            @Override
            public void run() {
...
//            mDeviceCrashInfo.put("ctype", Setting.getInstance().getCType());
    ...
    private String saveCrashInfoToFile(Throwable ex) {
        ...
        try {
            String fileName = "crash-" + DateTools.dateFormatterOfDateTimeForNow(DateTools.DATE_FORMAT_STYLE_1) + CRASH_REPORTER_EXTENSION;
