...
//					BaseApplication.getInstance().displayImage(holder.image,Constants.URL_PERFIX+bitmapPath);
            ...
            holder.delImage.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (context instanceof SgPushTieziDetailActivity) {
                        DialogFacory.getInstance().createAlertDialog(context, "提示", "您确认删除图片吗?", "确定", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                ((SgPushTieziDetailActivity) context).deleteImage(bitmapPath);
                            }
                        },null).show();
                    }else if(context instanceof SgPushGongQiuDetailActivity){
                        DialogFacory.getInstance().createAlertDialog(context, "提示", "您确认删除图片吗?", "确定", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                ((SgPushGongQiuDetailActivity) context).deleteImage(bitmapPath);
                            }
                        },null).show();
                    }
                }
