...
public class SgGuiQuanManagerOfficalActivity extends BaseActivity implements View.OnClickListener  {
...
//    private Toolbar mToolBar;
    ...
    protected void onResume() {
        ...
        if(UserPreference.getTOKEN()!=null&&UserPreference.getTOKEN().length()>1){
            GuiMiController.getInstance().CiecleContentDetail(SgGuiQuanManagerOfficalActivity.this, Integer.parseInt(model.getId()), UserPreference.getUid());
        }
...
//                    UserPreference.setTOPICCONTENT(model.getTitle());
                    if(QuanZiModel.getIs_in().equals("1")){
                        PopUpDialog();
                    }else{
...
//        });
        swipeLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                ...
                GuiMiController.getInstance().CircleMemberList(SgGuiQuanManagerOfficalActivity.this, Integer.parseInt(model.getId()), 0, 3);
            }
        });
        ...
        personInfoIv.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(!StringTools.isNullOrEmpty(model.getAvatar())){
                    IntentTools.startBigImage(SgGuiQuanManagerOfficalActivity.this,model.getAvatar());
                }
            }
        });
    }
        ...
        if (flag == HttpConfig.Circlepprove.getType()) {
            ...
            try {
                ...
                if(json.getJSONObject("main")!=null){
                    ...
                    reservedLayout.findViewById(R.id.personInfoIvItemd).setOnClickListener(SgGuiQuanManagerOfficalActivity.this);
        ...
        if(flag==HttpConfig.ApplyJoinCircle.getType()){
            ...
            try {
                ...
                if (object.getBoolean("status")) {
                    if(model.getMember_audit().equals("1")){
                        //无需审核
                        ToastTool.show("加入成功");
                        GuiMiController.getInstance().CiecleContentDetail(this, Integer.parseInt(QuanZiModel.getId()), UserPreference.getUid());
                    }else{
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(resultCode==RESULT_OK&&requestCode==123){
            ...
            if(data.getSerializableExtra("variety_id")!=null){
                String ds=(String)data.getSerializableExtra("variety_id");
                if(ds.equals("1")){
                    finish();
                }
            }
...
//                IntentTools.startquanziDetail(this,model,true);
                if(UserPreference.getTOKEN()!=null&&UserPreference.getTOKEN().length()>1){
                    if (Integer.parseInt(QuanZiModel.getUser_id()) == UserPreference.getUid()) {
                        ...
                        if(Integer.parseInt(QuanZiModel.getIs_in())==0){
                            IntentTools.startQuanziBackDetailSelf(this,QuanZiModel,123);
                        }else{
                            IntentTools.startQuanziBackDetailSelf(this,QuanZiModel,123);
                        }
                    }
            ...
            case R.id.allLayout:
                if(QuanZiModel!=null){
                    IntentTools.startNewMember(this,QuanZiModel);
                }else{
