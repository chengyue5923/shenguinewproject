 ...
 */
public class ActivityQuanZiNumberAddMoreListAdapter extends BasePlatAdapter<CircleMemberDetail> {
    Context mContext;
    QuanziList Circlem;
    public ActivityQuanZiNumberAddMoreListAdapter(Context context, QuanziList model) {
        super(context);
        mContext=context;
        Circlem=model;
    }
    ...
    public View getView(final int position, View itemView, ViewGroup parent) {
        ...
        if (itemView == null) {
            vh = new ViewHolder();
            itemView = LayoutInflater.from(mContext).inflate(R.layout.view_quanzi_member_list_item_add_more, null);
            ...
            public void onClick(View v) {
                ...
                try{
                    if(UserPreference.getTOKEN()!=null&&UserPreference.getTOKEN().length()>1){
                        GuiMiController.getInstance().CircleDeleteMemberList(new ViewNetCallBack() {
                            @Override
